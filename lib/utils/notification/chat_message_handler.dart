...
/*  static void add(Widget chat) {
  ...
  static void addchat(Widget chat) {
    dynamic chatId;
    if (chat is ChatMessage && chat.key is ValueKey) {
      chatId = (chat.key as ValueKey).value;
    } else if (chat is BlocProvider && (chat).child is ChatMessage) {
      var child = (chat).child as ChatMessage;
      if (child.key is ValueKey) chatId = (child.key as ValueKey).value;
    }

    if (chatId != null) {
      bool exists = messages.any((element) {
        dynamic elementId;
